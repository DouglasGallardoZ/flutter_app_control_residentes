...
class _AdminFacialEnrollmentPageState extends State<AdminFacialEnrollmentPage> {
          ...
          actions: [
            TextButton(
              onPressed: () {
                ...
                String mensaje = 'Residente registrado correctamente';
                if (widget.type == 'member') {
                  mensaje = 'Miembro de familia registrado correctamente';
          ...
          actions: [
            TextButton(
              onPressed: () {
                Navigator.of(ctx).pop();
                _disposeCamera();
                context.read<FacialEnrollmentBloc>().add(EnrollmentRetried());
                Navigator.of(context).pushReplacementNamed(
                  '/adminDashboard',
                  arguments: {
                    'personaId': widget.personaId,
  ...
  Widget _buildContent(FacialEnrollmentState state) {
    ...
    final Map<FaceAngle, Uint8List?> imagenes = state is FacialEnrollmentInProgress
        ? state.imagenes
        : <FaceAngle, Uint8List?>{};
    ...
    return _buildMobileLayout(cameraView, imagenes, poseActual,
        instruccion, fotosCapturadas, hayError);
  }
    ...
    return Stack(
      children: [
        ...
        Positioned(
          ...
          left: 16,
          right: 16,
          child: _buildMobileInstruction(instruccion, fotosCapturadas),
        ),
        Positioned(
          bottom: 20,
          left: 16,
          child: FloatingActionButton(
            heroTag: 'close_mobile',
                ...
                child: Center(
                  child: capturada
                      ...
                      : Icon(_iconoPose(angulo),
                          color:
                              esActual ? Colors.cyanAccent : Colors.white54,
                          size: 20),
                ),
              ),
  ...
  Widget _buildMobileInstruction(String instruccion, int fotosCapturadas) {
    return Container(
            ...
            Text(
              ...
              style: TextStyle(
                ...
                fontWeight:
                    esActual ? FontWeight.bold : FontWeight.normal,
                color:
                    capturada ? Colors.green : Colors.grey.shade700,
              ),
            ),
          ],
        );
