...
class CreateSpousePage extends StatefulWidget {
  ...
  @override
  State<CreateSpousePage> createState() => _CreateSpousePageState();
}

class _CreateSpousePageState extends State<CreateSpousePage> {
  ...
  void _createSpouse() {
    ...
    context.read<OwnerBloc>().add(
          CreateSpouseEvent(
            ...
            direccionAlternativa: _direccionAlternativaController.text.isEmpty
                ? null
      ...
      body: BlocListener<OwnerBloc, OwnerState>(
        listener: (context, state) {
          if (state is SpouseCreated) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Cónyuge registrado exitosamente'),
                behavior: SnackBarBehavior.floating,
              ),
            );
          ...
          } else if (state is SpouseError) {
            _showErrorSnackBar(state.message);
          }
        },
        child: BlocBuilder<OwnerBloc, OwnerState>(
          builder: (context, state) {
            final isLoading = state is SpouseCreating;
                            ...
                            children: [
                              ...
                              Row(
                                children: [
                                  ...
                                  Expanded(
                                    child: FilledButton(
                                      onPressed: isLoading ? null : _createSpouse,
                                      child: isLoading
                                          ? const SizedBox(
                                              height: 20,
                                              width: 20,
